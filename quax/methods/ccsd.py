...
def rccsd(
    ...
    # Do HF
    E_scf, C, eps, V = restricted_hartree_fock(
        geom,
        basis_name,
        xyz_path,
        nuclear_charges,
    ...
    D = 1.0 / (
        fock_Od.reshape(-1, 1, 1, 1)
        + fock_Od.reshape(-1, 1, 1)
        - fock_Vd.reshape(-1, 1)
        - fock_Vd
    )
...
def rccsd_energy(T1, T2, Voovv):
    E_ccsd = 0.0
    E_ccsd -= jnp.tensordot(
        T1, jnp.tensordot(T1, Voovv, [(0, 1), (1, 2)]), [(0, 1), (0, 1)]
    )
    E_ccsd -= jnp.tensordot(T2, Voovv, [(0, 1, 2, 3), (1, 0, 2, 3)])
    E_ccsd += 2.0 * jnp.tensordot(T2, Voovv, [(0, 1, 2, 3), (0, 1, 2, 3)])
    E_ccsd += 2.0 * jnp.tensordot(
        T1, jnp.tensordot(T1, Voovv, [(0, 1), (0, 2)]), [(0, 1), (0, 1)]
    )
    return E_ccsd
...
def rccsd_iter(T1, T2, V, d, D, ndocc, nvir):
    ...
    newT2 += 2.0 * jnp.einsum(
        "ikac, jlbd, klcd -> ijab", T2, T2, Voovv, optimize="optimal"
    )
    newT2 *= 2.0
    ...
    newT2 -= jnp.einsum(
        "ic, jd, ka, kbcd -> ijab", T1, T1, T1, Vovvv, optimize="optimal"
    )
    newT2 -= jnp.einsum(
        "ic, jd, kb, kadc -> ijab", T1, T1, T1, Vovvv, optimize="optimal"
    )
    newT2 += jnp.einsum(
        "ic, ka, lb, lkjc -> ijab", T1, T1, T1, Vooov, optimize="optimal"
    )
    newT2 += jnp.einsum(
        "jc, ka, lb, klic -> ijab", T1, T1, T1, Vooov, optimize="optimal"
    )
    ...
    newT2 += jnp.einsum(
        "ic, jd, ka, lb, klcd -> ijab", T1, T1, T1, T1, Voovv, optimize="optimal"
    )
    newT2 += jnp.einsum(
        "ic, jd, lkab, lkcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    newT2 += jnp.einsum(
        "ka, lb, ijdc, lkcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    ...
    P_OVVO -= jnp.einsum(
        "kc, jd, ilab, klcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    P_OVVO -= jnp.einsum(
        "kc, la, ijdb, klcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    P_OVVO -= jnp.einsum(
        "ic, ka, jlbd, klcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    ...
    P_OVVO += jnp.einsum(
        "kc, jd, ilab, lkcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    P_OVVO += jnp.einsum(
        "kc, la, ijdb, lkcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    P_OVVO += jnp.einsum(
        "ic, ka, ljbd, klcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    P_OVVO += jnp.einsum(
        "ic, ka, ljdb, lkcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
    P_OVVO += jnp.einsum(
        "ic, lb, kjad, klcd -> ijab", T1, T1, T2, Voovv, optimize="optimal"
    )
