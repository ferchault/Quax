        ...
        if algo == "libint_core":
            ...
            if check:
                ...
                if deriv_order <= 2:
                    ...
                    basis_dict = build_basis_set(molecule, basis_name)
                    S, T, V = oei_arrays(
                        geom.reshape(-1, 3), basis_dict, nuclear_charges
                    )
        elif deriv_order == 0:
            libint_interface.initialize(xyz_path, basis_name)
            tei_obj = TEI(basis_name, xyz_path, deriv_order, "core")
...
def check_disk(geom, basis_name, xyz_path, deriv_order, address=None):
    ...
    if os.path.exists("eri_derivs.h5") and os.path.exists("oei_derivs.h5"):
        print(
            "Found currently existing integral derivatives in your working directory. Trying to use them."
        )
    ...
    # for particular address
        ...
        print(
            "Found currently existing partial derivatives in working directory. Assuming they are correct."
        )
...
def write_integrals(molecule, basis_name, deriv_order, address):
    ...
    for deriv_vec in deriv_vecs:
        ...
        if order == 1:
            ...
            dS, dT, dV = jacfwd(jacfwd(jacfwd(oei_wrapper, i), j), k)(
                *geom_list, **kwargs
            )
            dG = jacfwd(jacfwd(jacfwd(tei_wrapper, i), j), k)(*geom_list, **kwargs)
        elif order == 4:
            i, j, k, l = address[0], address[1], address[2], address[3]
            dS, dT, dV = jacfwd(jacfwd(jacfwd(jacfwd(oei_wrapper, i), j), k), l)(
                *geom_list, **kwargs
            )
            dG = jacfwd(jacfwd(jacfwd(jacfwd(tei_wrapper, i), j), k), l)(
                *geom_list, **kwargs
            )
        elif order == 5:
            i, j, k, l, m = address[0], address[1], address[2], address[3], address[4]
            dS, dT, dV = jacfwd(
                jacfwd(jacfwd(jacfwd(jacfwd(oei_wrapper, i), j), k), l), m
            )(*geom_list, **kwargs)
            dG = jacfwd(jacfwd(jacfwd(jacfwd(jacfwd(tei_wrapper, i), j), k), l), m)(
                *geom_list, **kwargs
            )
        elif order == 6:
            i, j, k, l, m, n = (
            ...
            dS, dT, dV = jacfwd(
                jacfwd(jacfwd(jacfwd(jacfwd(jacfwd(oei_wrapper, i), j), k), l), m), n
            )(*geom_list, **kwargs)
            dG = jacfwd(
                jacfwd(jacfwd(jacfwd(jacfwd(jacfwd(tei_wrapper, i), j), k), l), m), n
            )(*geom_list, **kwargs)
