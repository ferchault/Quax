...
def primitive_tei(La, Lb, Lc, Ld, A, B, C, D, aa, bb, cc, dd, c1, c2, c3, c4):
    ...
    with loops.Scope() as s:
        ...
        for _ in s.while_range(lambda: s.I < la + lb + lc + ld + 1):
            ...
            for _ in s.while_range(lambda: s.J < ma + mb + mc + md + 1):
                ...
                for _ in s.while_range(lambda: s.K < na + nb + nc + nd + 1):
                    s.primitive += (
                        Bx[s.I] * By[s.J] * Bz[s.K] * boys_eval[s.I + s.J + s.K]
                    )
                    s.K += 1
                s.J += 1
            s.I += 1
        value = (
            2
            * jax.lax.pow(jnp.pi, 2.5)
            / (gamma1 * gamma2 * jnp.sqrt(gamma1 + gamma2))
            * jnp.exp(-aa * bb * rab2 / gamma1)
            * jnp.exp(-cc * dd * rcd2 / gamma2)
            * s.primitive
            * coef
        )
        return value
...
def tei_array(geom, basis):
    ...
    primitive_quartets = cartesian_product(
        jnp.arange(nprim), jnp.arange(nprim), jnp.arange(nprim), jnp.arange(nprim)
    )
    ...
    aa_plus_bb = jnp.broadcast_to(exps, (nprim, nprim)) + jnp.transpose(
        jnp.broadcast_to(exps, (nprim, nprim)), (1, 0)
    )
    ...
    PminusA_pow = jnp.power(
        jnp.transpose(
            jnp.broadcast_to(PminusA, (max_am + 1, nprim, nprim, natom, 3)),
            (1, 2, 3, 4, 0),
        ),
        jnp.arange(max_am + 1),
    )
    ...
    with loops.Scope() as s:
        ...
        for prim_quar in s.range(primitive_quartets.shape[0]):
            ...
            ld1, ld2, ld3, ld4 = (
                am_leading_indices[ams[p1]],
                am_leading_indices[ams[p2]],
                am_leading_indices[ams[p3]],
                am_leading_indices[ams[p4]],
            )
            idx1, idx2, idx3, idx4 = (
                indices[p1],
                indices[p2],
            ...
            QP_pow = jnp.power(
                jnp.broadcast_to(Q - P, (max_am_idx, 3)).T, jnp.arange(max_am_idx)
            )
            # Gamma powers are negative, up to -(l1+l2).
            # Make array such that the given negative index returns the same negative power.
            g1_pow = jnp.power(
                4 * gamma1, -jnp.roll(jnp.flip(jnp.arange(2 * max_am + 1)), 1)
            )
            g2_pow = jnp.power(
                4 * gamma2, -jnp.roll(jnp.flip(jnp.arange(2 * max_am + 1)), 1)
            )
            oodelta_pow = jnp.power(
                1 / delta, jnp.arange(max_am_idx)
            )  # l1 + l2 + l3 + l4 + 1

            prefactor = (
                34.986836655249726
                / (gamma1 * gamma2 * jnp.sqrt(gamma1 + gamma2))
                * jnp.exp(-aa * bb * rab2 / gamma1 + -cc * dd * rcd2 / gamma2)
                * coef
            )
                        ...
                        for _ in s.while_range(lambda: s.d < dims[p4]):
                            ...
                            with loops.Scope() as S:
                                ...
                                for _ in S.while_range(
                                    lambda: S.I < la + lb + lc + ld + 1
                                ):
                                    S.J = 0
                                    tmp = Bx[S.I]
                                    for _ in S.while_range(
                                        lambda: S.J < ma + mb + mc + md + 1
                                    ):
                                        S.K = 0
                                        tmp *= By[S.J]
                                        for _ in S.while_range(
                                            lambda: S.K < na + nb + nc + nd + 1
                                        ):
