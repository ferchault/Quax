...
def overlap_component(l1, l2, PAx, PBx, gamma):
    ...
    with loops.Scope() as s:
        ...
        for _ in s.while_range(lambda: s.i < K):
            s.total += (
                binomial_prefactor(2 * s.i, l1, l2, PAx, PBx)
                * double_factorials[2 * s.i - 1]
                / (2 * gamma) ** s.i
            )
            s.i += 1
        return s.total
...
def kinetic(la, ma, na, lb, mb, nb, aa, bb, PA_pow, PB_pow, prefactor):
    ...
    term2 = (
        -2 * bb ** 2 * (wx_plus2 * wy * wz + wx * wy_plus2 * wz + wx * wy * wz_plus2)
    )

    term3 = -0.5 * (
        lb * (lb - 1) * wx_minus2 * wy * wz
...
def A_array(l1, l2, PA, PB, CP, g, A_vals):
    with loops.Scope() as s:
        ...
        for _ in s.while_range(lambda: s.i > -1):
            Aterm = (
                neg_one_pow[s.i]
                * binomial_prefactor(s.i, l1, l2, PA, PB)
                * factorials[s.i]
            )
            s.r = s.i // 2
            for _ in s.while_range(lambda: s.r > -1):
                ...
                for _ in s.while_range(lambda: s.u > -1):
                    ...
                    tmp = I - s.u
                    fact_ratio = 1 / (
                        factorials[s.r] * factorials[s.u] * factorials[tmp]
                    )
                    Aterm *= (
                        neg_one_pow[s.u]
                        * CP[tmp]
...
def potential(
    ...
    with loops.Scope() as s:
        ...
        for i in s.range(Pgeom_pow.shape[0]):
            Ax = A_array(
                la, lb, PA_pow[0], PB_pow[0], Pgeom_pow[i, 0, :], gamma, A_vals
            )
            Ay = A_array(
                ma, mb, PA_pow[1], PB_pow[1], Pgeom_pow[i, 1, :], gamma, A_vals
            )
            Az = A_array(
                na, nb, PA_pow[2], PB_pow[2], Pgeom_pow[i, 2, :], gamma, A_vals
            )
...
def oei_arrays(geom, basis, charges):
    ...
    with loops.Scope() as s:
        ...
        for prim_duet in s.range(primitive_duets.shape[0]):
            ...
            PA_pow = jnp.power(
                jnp.broadcast_to(P - A, (max_am + 3, 3)).T, jnp.arange(max_am + 3)
            )
            PB_pow = jnp.power(
                jnp.broadcast_to(P - B, (max_am + 3, 3)).T, jnp.arange(max_am + 3)
            )
            ...
            Pgeom_pow = jnp.power(
                jnp.transpose(
                    jnp.broadcast_to(
                        P_minus_geom, (2 * max_am + 1, geom.shape[0], geom.shape[1])
                    ),
                    (1, 2, 0),
                ),
                jnp.arange(2 * max_am + 1),
            )
            ...
            for _ in s.while_range(lambda: s.a < dims[p1]):
                ...
                for _ in s.while_range(lambda: s.b < dims[p2]):
                    ...
                    overlap_int = (
                        overlap(
                            la, ma, na, lb, mb, nb, aa, bb, PA_pow, PB_pow, prefactor
                        )
                        * coef
                    )
                    kinetic_int = (
