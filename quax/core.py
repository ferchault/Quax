    ...
    for key in options.keys():
        if key in keyword_options.keys():
            if type(options[key]) == type(keyword_options[key]):
                ...
                print(
                    "Value '{}' for keyword option '{}' not recognized. Ignoring.".format(
                        options[key], key
                    )
                )
        else:
...
def compute(molecule, basis_name, method, options=None, deriv_order=0, partial=None):
    ...
    if not partial:
        ...
        if method == "scf" or method == "hf" or method == "rhf":
            ...
            print(
                "Desired electronic structure method not understood. Use 'scf' 'hf' 'mp2' 'ccsd' or 'ccsd(t)' "
            )
        ...
        if deriv_order == 0:
            ...
            print(
                "Error: Order {} derivatives are not exposed to the API.".format(
                    deriv_order
                )
            )
            deriv = 0
    ...
    # Partial derivatives
        ...
        if len(partial) != deriv_order:
            raise Exception(
                "The length of the index coordinates given by 'partial' argument should be the same as the order of differentiation"
            )
        ...
        print(
            "Estimated memory footprint from two-electron integral partial derivatives: {} GB".format(
                ngigabytes
            )
        )
        ...
        if deriv_order == 1:
            ...
            partial_deriv = jacfwd(jacfwd(jacfwd(jacfwd(partial_wrapper, i), j), k), l)(
                *geom_list
            )
        elif deriv_order == 5:
            i, j, k, l, m = partial[0], partial[1], partial[2], partial[3], partial[4]
            partial_deriv = jacfwd(
                jacfwd(jacfwd(jacfwd(jacfwd(partial_wrapper, i), j), k), l), m
            )(*geom_list)
        elif deriv_order == 6:
            i, j, k, l, m, n = (
                partial[0],
            ...
            partial_deriv = jacfwd(
                jacfwd(jacfwd(jacfwd(jacfwd(jacfwd(partial_wrapper, i), j), k), l), m),
                n,
            )(*geom_list)
        else:
            print(
                "Error: Order {} partial derivatives are not exposed to the API.".format(
                    deriv_order
                )
            )
            partial_deriv = 0
...
def partial_derivative(
    molecule, basis_name, method, deriv_order, partial, options=None
):
    """
