...
class OEI(object):
    def __init__(self, basis_name, xyz_path, max_deriv_order, mode):
        ...
        jax.interpreters.batching.primitive_batchers[
            self.overlap_deriv_p
        ] = self.overlap_deriv_batch
        jax.interpreters.batching.primitive_batchers[
            self.kinetic_deriv_p
        ] = self.kinetic_deriv_batch
        jax.interpreters.batching.primitive_batchers[
            self.potential_deriv_p
        ] = self.potential_deriv_batch
        ...
        if self.mode == "core":
            ...
            with h5py.File(file_name, "r") as f:
                ...
                if len(data_set.shape) == 3:
                    ...
                    raise Exception(
                        "Something went wrong reading integral derivative file"
                    )
            return jnp.asarray(S)
