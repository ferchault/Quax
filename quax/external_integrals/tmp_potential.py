...
from ..integrals.integrals_utils import (
    boys,
    binomial_prefactor,
    gaussian_product,
    boys,
...
def tmp_potential(geom, basis, charges):
    ...
    with loops.Scope() as s:
        ...
        for prim_duet in s.range(primitive_duets.shape[0]):
            ...
            PA_pow = np.power(
                np.broadcast_to(P - A, (max_am + 3, 3)).T, np.arange(max_am + 3)
            )
            PB_pow = np.power(
                np.broadcast_to(P - B, (max_am + 3, 3)).T, np.arange(max_am + 3)
            )
